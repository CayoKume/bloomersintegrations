...
namespace BloomersWorkers.LabelsPrinter.Infrastructure.Repositorys
{
    ...
    public class LabelsPrinterRepository : ILabelsPrinterRepository
    {
        ...
        public async Task<List<Order>> GetJadlogOrders()
        {
            try
            {
                var sql = $@"SELECT DISTINCT
                             ...
                             A.NB_ENTREGAR_PARA AS DELIVERU_TO,
                             A.SERIE AS SERIE,
                             ...
                             (IIF(
		                            (SELECT DISTINCT TOP 1 SHIPMENTID FROM GENERAL..JADLOGREGISTROLOG T1 WHERE T1.PEDIDO = TRIM(A.DOCUMENTO) AND T1.DESCRICAO LIKE '%JÁ FOI ENVIADO%') = '', 
		                            (SELECT SUBSTRING (
							                            (SELECT DISTINCT TOP 1 DESCRICAO FROM GENERAL..JADLOGREGISTROLOG T2 WHERE T2.PEDIDO = TRIM(A.DOCUMENTO)),
							                            CHARINDEX(
							 			                        'SHIPMENTID: ', 
							 			                        (SELECT DISTINCT TOP 1 DESCRICAO FROM GENERAL..JADLOGREGISTROLOG T2 WHERE T2.PEDIDO = TRIM(A.DOCUMENTO))) + 12,
							 			                        LEN((SELECT DISTINCT TOP 1 DESCRICAO FROM GENERAL..JADLOGREGISTROLOG T2 WHERE T2.PEDIDO = TRIM(A.DOCUMENTO)))
							                            )
		                            ),
		                            (SELECT DISTINCT TOP 1 SHIPMENTID FROM GENERAL..JADLOGREGISTROLOG T1 WHERE T1.PEDIDO = TRIM(A.DOCUMENTO))
	                             )
                             ) AS SHIPMENTID,
                ...
                var result = await _conn.GetIDbConnection().QueryAsync<Order, Client, ShippingCompany, Company, Invoice, Order>(sql, (order, client, shippingCompany, company, invoice) =>
                {
                    order.client = client;
            ...
            {
                throw new Exception($"GetJadlogOrders - Erro ao obter pedidos na tabela IT4_WMS_DOCUMENTO  - {ex.Message}");
            }
        }
