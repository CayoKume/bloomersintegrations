...
namespace BloomersCommerceIntegrations.LinxCommerce.Application.Services
{
    public class OrderService<TEntity> : IOrderService<TEntity> where TEntity : SearchOrderResponse.Root, new()
    {
        ...
        public async Task IntegraRegistros(string database)
        {
            ...
            //        {
            //            Page = new { PageIndex = z, PageSize = 500 },
            ...
            //                var orderCustomerResponse = String.Empty;
            //                if (order.CustomerCPF != null)
            //                    orderCustomerResponse = await _apiCall.PostRequest(registro.CustomerID, "/v1/Profile/API.svc/web/GetPerson", AUTENTIFICACAO, CHAVE);
            //                else if (order.CustomerCNPJ != null)
            //                    orderCustomerResponse = await _apiCall.PostRequest(registro.CustomerID, "/v1/Profile/API.svc/web/GetCustomer", AUTENTIFICACAO, CHAVE);
            ...
            try
            {
                var pedidos = await _orderRepository.GetRegistersExists(new List<string>(), database);
                    ...
                    if (searchOrders.Result.Count() > 0)
                    {
                        ...
                        foreach (var registro in searchOrders.Result)
                        {
                            ...
                            if (order.CustomerCPF != null)
                                ...
                                orderCustomerResponse = await _apiCall.PostRequest(registro.CustomerID, "/v1/Profile/API.svc/web/GetCompany", AUTENTIFICACAO, CHAVE);
