...
namespace BloomersCommerceIntegrations.LinxCommerce.Infrastructure.Repositorys
{
    public class SKURepository : ISKURepository
    {
        ...
        public void BulkInsertIntoTableRaw(List<SKUs> registros, string? database)
        {
            try
            {
                var sku = CreateDataTable("Sku", new List<string> { "UPC", "DisplayCondition", "DefinitionID", "ConditionID", "UnitOfMeasureID", "ManageStock", "MinimumQtyAllowed", "MaximumQtyAllowed", "Preorderable", "PreorderDate", "PreorderLimit", "Backorderable", "BackorderLimit", "PurchasingPolicyID", "WrappingQty", "Weight", "Height", "Width", "Depth", "Cost", "Tax", "IsVisible", "VisibleFrom", "VisibleTo", "IsDeleted", "ProductID", "Name", "SKU", "CreatedDate", "CreatedBy", "ModifiedDate", "ModifiedBy", "IntegrationID", "ParentID", "lastupdateon" });
                ...
                var skuMetaDataValue = CreateDataTable("SkuMetaDataValue", new List<string> { "DisplayName","FormattedValue","InputType","IntegrationID","PropertyGroup","PropertyMetadataID","PropertyName","SerializedBlobValue","SerializedValue","Value","ProductID","lastupdateon" });
        ...
        public void BulkInsertIntoTableRaw(SearchSKUResponse.Root registros, string? database)
        {
            try
            {
                var skuBase = CreateDataTable("SkuBase", new List<string> { "CreatedDate","IntegrationID","ModifiedDate","Name","ProductID","SKU","lastupdateon" });
        ...
        public async Task<List<SKUs>> GetRegistersExists(List<string> productsIds, string? database)
        {
            ...
            string query = @$"SELECT A.PRODUCTID FROM LINX_COMMERCE..SKUBASE_TRUSTED A (NOLOCK)
                              LEFT JOIN LINX_COMMERCE..SKU_TRUSTED B (NOLOCK) ON A.PRODUCTID = B.PRODUCTID
