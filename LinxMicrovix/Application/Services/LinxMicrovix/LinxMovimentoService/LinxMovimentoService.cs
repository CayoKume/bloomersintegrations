...
{
    public class LinxMovimentoService<T1> : ILinxMovimentoService<T1> where T1 : LinxMovimento, new()
    {
        ...
        private readonly ILinxMovimentoRepository<LinxMovimento> _linxMovimentoRepository;

        public LinxMovimentoService(ILinxMovimentoRepository<LinxMovimento> linxMovimentoRepository)
        ...
        public async Task IntegraRegistros(string tableName, string procName, string database)
        {
            try
            {
                var cnpjs = await _linxMovimentoRepository.GetEmpresas();

                foreach (var cnpj in cnpjs)
                {
                    PARAMETERS = await _linxMovimentoRepository.GetParameters(tableName, "parameters_lastday");
                    ...
                    if (registros.Count() > 0)
                    {
                        ...
                        {
                            var list = listResults.ConvertAll(new Converter<T1, LinxMovimento>(T1ToObject));
                            _linxMovimentoRepository.BulkInsertIntoTableRaw(list, tableName, database);
                            //await _linxMovimentoRepository.CallDbProcMerge(procName, tableName, database);
                        }
                    }
                };
            }
        ...
        public void IntegraRegistrosSync(string tableName, string procName, string database)
        {
            try
            {
                var cnpjs = _linxMovimentoRepository.GetEmpresasSync();

                foreach (var cnpj in cnpjs)
                {
                    PARAMETERS = _linxMovimentoRepository.GetParametersSync(tableName, "parameters_lastday");
                    ...
                    if (registros.Count() > 0)
                    {
                        ...
                        if (listResults.Count() > 0)
                        {
                            var list = listResults.ConvertAll(new Converter<T1, LinxMovimento>(T1ToObject));
                            _linxMovimentoRepository.BulkInsertIntoTableRaw(list, tableName, database);
                            //_linxMovimentoRepository.CallDbProcMergeSync(procName, tableName, database);
                        }
                    }
                };
            }
        ...
        public async Task<bool> IntegraRegistrosIndividual(string tableName, string procName, string database, string identificador, string cnpj_emp)
        {
            try
            {
                PARAMETERS = await _linxMovimentoRepository.GetParameters(tableName, "parameters_manual");

                string response = APICaller.CallLinxAPI(PARAMETERS.Replace("[documento]", $"{identificador}").Replace("[data_fim]", $"{DateTime.Today.ToString("yyyy-MM-dd")}").Replace("[0]", "0"), tableName, AUTENTIFICACAO, CHAVE, cnpj_emp);
                var registros = APICaller.DeserializeXML(response);
                var registro = DeserializeResponse(registros);

                if (registro.Count() > 0)
                {
                    await _linxMovimentoRepository.InsereRegistroIndividual(registro[0], tableName, database);
                    //await _linxMovimentoRepository.CallDbProcMerge(procName, tableName, database);
                    return true;
                }
                else
        ...
        public bool IntegraRegistrosIndividualSync(string tableName, string procName, string database, string identificador, string cnpj_emp)
        {
            try
            {
                PARAMETERS = _linxMovimentoRepository.GetParametersSync(tableName, "parameters_manual");
                ...
                if (registro.Count() > 0)
                {
                    _linxMovimentoRepository.InsereRegistroIndividualSync(registro[0], tableName, database);
                    //_linxMovimentoRepository.CallDbProcMergeSync(procName, tableName, database);
                    return true;
                }
                else
        ...
        public T1? T1ToObject(T1 t1)
        {
            ...
            {
                throw new Exception($"LinxMovimento - T1ToObject - Erro ao converter registro: {t1.documento} para objeto - {ex.Message}");
            }
        }
    }
}
