...
{
    public class LinxClientesFornecService<T1> : ILinxClientesFornecService<T1> where T1 : LinxClientesFornec, new()
    {
        ...
        private readonly ILinxClientesFornecRepository<LinxClientesFornec> _linxClientesFornecRepository;

        public LinxClientesFornecService(ILinxClientesFornecRepository<LinxClientesFornec> linxClientesFornecRepository)
        ...
        public List<T1?> DeserializeResponse(List<Dictionary<string, string>> registros)
        {
            ...
            for (int i = 0; i < registros.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception($"LinxClientesFornec - DeserializeResponse - Erro ao deserealizar registro: {registroComErro} - {ex.Message}");
                }
            }
        ...
        public async Task IntegraRegistros(string tableName, string procName, string database)
        {
            try
            {
                PARAMETERS = await _linxClientesFornecRepository.GetParameters(tableName, "parameters_lastday");
                ...
                if (registros.Count() > 0)
                {
                    ...
                    {
                        var list = listResults.ConvertAll(new Converter<T1, LinxClientesFornec>(T1ToObject));
                        _linxClientesFornecRepository.BulkInsertIntoTableRaw(list, tableName, database);
                        //await _linxClientesFornecRepository.CallDbProcMerge(procName, tableName, database);
                    }
                }
            }
            catch
        ...
        public void IntegraRegistrosSync(string tableName, string procName, string database)
        {
            try
            {
                PARAMETERS = _linxClientesFornecRepository.GetParametersSync(tableName, "parameters_lastday");
                ...
                if (registros.Count() > 0)
                {
                    ...
                    if (listResults.Count() > 0)
                    {
                        var list = listResults.ConvertAll(new Converter<T1, LinxClientesFornec>(T1ToObject));
                        _linxClientesFornecRepository.BulkInsertIntoTableRaw(list, tableName, database);
                        //_linxClientesFornecRepository.CallDbProcMergeSync(procName, tableName, database);
                    }
                }
            }
            catch
        ...
        public async Task<bool> IntegraRegistrosIndividual(string tableName, string procName, string database, string identificador)
        {
            try
            {
                PARAMETERS = await _linxClientesFornecRepository.GetParameters(tableName, "parameters_manual");
                ...
                if (cliente.Count() > 0)
                {
                    await _linxClientesFornecRepository.InsereRegistroIndividual(cliente[0], tableName, database);
                    //await _linxClientesFornecRepository.CallDbProcMerge(procName, tableName, database);
                    return true;
                }
                else
        ...
        public bool IntegraRegistrosIndividualSync(string tableName, string procName, string database, string identificador)
        {
            try
            {
                PARAMETERS = _linxClientesFornecRepository.GetParametersSync(tableName, "parameters_manual");
                ...
                if (cliente.Count() > 0)
                {
                    _linxClientesFornecRepository.InsereRegistroIndividualSync(cliente[0], tableName, database);
                    //_linxClientesFornecRepository.CallDbProcMergeSync(procName, tableName, database);
                    return true;
                }
                else
        ...
        public T1? T1ToObject(T1 t1)
        {
            ...
            {
                throw new Exception($"LinxClientesFornec - T1ToObject - Erro ao converter registro: {t1.doc_cliente} para objeto - {ex.Message}");
            }
        }
    }
}
