...
namespace BloomersCarriersIntegrations.Jadlog.Infrastructure.Repositorys
{
    public class JadlogRepository : IJadlogRepository
    {
        ...
        public async Task GenerateResponseLog(string pedido, Response response)
        {
            var descricao = response.erro is null ? null : response.erro.descricao;

            var sql = @$"INSERT INTO [GENERAL].[dbo].JadlogRegistroLog (pedido, dataenvio, codigo, shipmentId, status, descricao) 
                         VALUES ('{pedido}', GETDATE(), '{response.codigo}', '{response.shipmentId}', '{response.status}', '{descricao}')";
            try
            {
                using (var conn = _conn.GetDbConnection())
                {
