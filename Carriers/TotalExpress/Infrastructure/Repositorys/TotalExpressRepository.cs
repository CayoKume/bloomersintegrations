    ...
    public class TotalExpressRepository : ITotalExpressRepository
    {
        ...
        public async Task<Order> GetInvoicedOrder(string nr_pedido)
        {
            var sql = $@"SELECT DISTINCT 
                                 ...
                                 A.NB_CFOP_PEDIDO AS CFOP_PEDIDO,
                                 ...
                                 B.CODIGO_BARRA AS COD_PRODUTO,
                                 B.NB_SKU_PRODUTO AS SKU_PRODUTO,
                                 B.DESCRICAO AS DESCRICAO_PRODUTO,
                                 B.CODIGO_BARRA AS COD_EAN_PRODUTO,
                                 B.QTDE AS QUANTIDADE_PRODUTO,
                                 B.NB_VALOR_UNITARIO_PRODUTO AS VALOR_UNITARIO_PRODUTO,
                                 B.NB_VALOR_TOTAL_PRODUTO AS VALOR_TOTAL_PRODUTO,
                                 B.NB_VALOR_FRETE_PRODUTO AS VALOR_FRETE_PRODUTO,

                                 A.NB_CODIGO_CLIENTE AS COD_CLIENTE,
                                 A.NB_RAZAO_CLIENTE AS RAZAO_CLIENTE,
                                 A.NB_NOME_CLIENTE AS NOME_CLIENTE,
                                 ...
                                 END AS NUMERO_RUA_CLIENTE,
                                 A.NB_COMPLEMENTO_END_CLIENTE AS COMPLEMENTO_ENDERECO_CLIENTE,
                                 ...
                                 A.NB_INSCRICAO_MUNICIPAL_CLIENTE AS INSCRICAO_MUNICIPAL_CLIENTE,

                                 A.NB_TRANSPORTADORA AS COD_TRANSPORTADORA,
                                 A.NB_METODO_TRANSPORTADORA AS METODO_TRANSPORTADORA,
                                 A.NB_RAZAO_TRANSPORTADORA AS RAZAO_TRANSPORTADORA,
                                 A.NB_NOME_TRANSPORTADORA AS NOME_TRANSPORTADORA,
                                 A.NB_DOC_TRANSPORTADORA AS DOC_TRANSPORTADORA,
                                 A.NB_EMAIL_TRANSPORTADORA AS EMAIL_TRANSPORTADORA,
                                 A.NB_ENDERECO_TRANSPORTADORA AS ENDERECO_TRANSPORTADOR,
                                 A.NB_NUMERO_RUA_TRANSPORTADORA AS NUMERO_RUA_TRANSPORTADORA,
                                 A.NB_COMPLEMENTO_END_TRANSPORTADORA AS COMPLEMENTO_END_TRANSPORTADORA,
                                 A.NB_BAIRRO_TRANSPORTADORA AS BAIRRO_TRANSPORTADORA,
                                 A.NB_CIDADE_TRANSPORTADORA AS CIDADE_TRANSPORTADORA,
                                 A.NB_UF_TRANSPORTADORA AS UF_TRANSPORTADORA,
                                 A.NB_CEP_TRANSPORTADORA AS CEP_TRANSPORTADORA,
                                 A.NB_FONE_TRANSPORTADORA AS FONE_TRANSPORTADORA,
                                 A.NB_INSCRICAO_ESTADUAL_TRANSPORTADORA AS INSCRICAO_ESTADUAL_TRANSPORTADORA,

                                 A.NB_COD_REMETENTE AS COD_EMPRESA,
                                 A.NB_RAZAO_REMETENTE AS RAZAO_EMPRESA,
                                 A.NB_NOME_REMETENTE AS NOME_EMPRESA,
                                 ...
                                 A.NB_EMAIL_REMETENTE AS EMAIL_EMPRESA,
                                 A.NB_ENDERECO_REMETENTE AS ENDERECO_EMPRESA,
                                 A.NB_NUMERO_RUA_REMETENTE AS NUMERO_RUA_EMPRESA,
                                 A.NB_COMPLEMENTO_END_REMETENTE AS COMPLEMENTO_END_EMPRESA,
                                 A.NB_BAIRRO_REMETENTE AS BAIRRO_EMPRESA,
                                 A.NB_CIDADE_REMETENTE AS CIDADE_EMPRESA,
                                 A.NB_UF_REMETENTE AS UF_EMPRESA,
                                 A.NB_CEP_REMETENTE AS CEP_EMPRESA,
                                 A.NB_FONE_REMETENTE AS FONE_EMPRESA,
                                 A.NB_INSCRICAO_ESTADUAL_REMETENTE AS INSCRICAO_ESTADUAL_EMPRESA,

                                 A.NF_SAIDA AS NUMERO_NF,
                                 A.NB_VALOR_PEDIDO AS TOTAL_NF,
                                 (SELECT CAST(SUBSTRING (A.[XML_FATURAMENTO], CHARINDEX('<vFrete>', CAST(A.[XML_FATURAMENTO] AS VARCHAR(MAX))) + 8, 4) AS DECIMAL(14,2))) AS FRETE_NF,
                                 (SELECT CAST(SUBSTRING (A.[XML_FATURAMENTO], CHARINDEX('<pesoB>', CAST(A.[XML_FATURAMENTO] AS VARCHAR(MAX))) + 7, 4) AS DECIMAL(14,2))) AS PESO_NF,
                                 A.CHAVE_NFE AS CHAVE_NFE_NF,
                                 CAST(A.[XML_FATURAMENTO] AS VARCHAR(MAX)) AS XML_NF,
                                 CAST(A.[XML_FATURAMENTO] AS VARCHAR(MAX)) AS XML_DISTRIBUICAO_NF,
                                 'NF' as TIPO_NF,
                                 (SELECT SUBSTRING (A.[XML_FATURAMENTO], CHARINDEX('<serie>', A.[XML_FATURAMENTO]) + 7, 1)) AS SERIE_NF,
                                 (SELECT SUBSTRING (A.[XML_FATURAMENTO], CHARINDEX('<dhEmi>', A.[XML_FATURAMENTO]) + 7, 25)) AS DATA_EMISSAO_NF
            ...
            try
            {
                using (var conn = _conn.GetDbConnection())
                {
                    var result = await conn.QueryAsync<Order, Product, Client, ShippingCompany, Company, Invoice, Order>(sql, (order, product, client, shippingCompany, company, invoice) =>
                    {
                        ...
                        return order;
                    }, splitOn: "cod_produto, cod_cliente, cod_transportadora, cod_empresa, numero_nf", commandTimeout: 360);
        ...
        public async Task<List<Order>> GetInvoicedOrders()
        {
            var sql = $@"SELECT DISTINCT 
                            ...
                            (SELECT REMETENTE_ID FROM GENERAL..PARAMETROS_TOTALEXPRESS WITH (NOLOCK, NOWAIT) WHERE PRODUTO =  
                                CASE 
                                WHEN A.DOCUMENTO LIKE '%OA-LJ%' THEN 'OPEN ERA - B2B'
                                WHEN A.DOCUMENTO LIKE '%MI-LJ%' THEN 'MISHA - B2B'
                            ...
                            (SELECT SERVICO_TIPO FROM GENERAL..PARAMETROS_TOTALEXPRESS WITH (NOLOCK, NOWAIT) WHERE PRODUTO =  
                            CASE 
                                WHEN A.DOCUMENTO LIKE '%OA-LJ%' THEN 'OPEN ERA - B2B'
                                WHEN A.DOCUMENTO LIKE '%MI-LJ%' THEN 'MISHA - B2B'
